import { StyleSheet, Text, View, Pressable, FlatList, TouchableOpacity, Image, useWindowDimensions } from 'react-native';
...
const ChatsListScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        ...
        const userQuery = query(collection(db, "users"), where('__name__', '!=', uid), orderBy('__name__'));
    ...
    const renderItem = ({ item }) => (
        <TouchableOpacity
            ...
            onPress={() => {
                ...
                navigation.navigate('Chat Room', { collectionId: collectionId, 
        ...
        >
            <View style={styles.listItem}>
                <View style={{ flexDirection: "row" }}>
                    { item.icon !== '' && <Image style={styles.avatar} source={{url: item.icon}} /> }
                    { item.icon === '' && <Image style={styles.avatar} source={{url: blankAvatarUrl}} /> }
    ...
    <TabBar
        {...props}
        indicatorStyle={{ backgroundColor: '#C5F277', height: 5, borderRadius: 5 }} 
        indicatorContainerStyle={{backgroundColor: '#blue'}}
        style={{ backgroundColor: 'black', borderRadius: 10}}
        renderLabel={({ route }) => (
            <Text style={{ color: '#C5F277', fontSize: 20 }}>
                {route.title}
            </Text>
        )}
    />
    ...
    return (
        <View style={styles.container}>
            <Pressable style={styles.backArrow} onPress={() => { navigation.popToTop(); }}>
